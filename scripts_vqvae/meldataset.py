...
TRAIN_DATA_MEAN = -9.6824
TRAIN_DATA_STD = 2.1489
...
def mel_spectrogram(
    ...
    # Add extra one sample on the end of the last dimension
    if mel_spec.shape[-1] % 2 != 0:
        mel_spec = torch.cat((mel_spec, mel_spec[:, :, -1:]), dim=-1)
        # print(f"Shape of mel_spec after adding extra sample: {mel_spec.shape}")
        
    # Scale the log-scale mel spectrogram to N(0, 1)
    mel_spec = (mel_spec - TRAIN_DATA_MEAN) / TRAIN_DATA_STD # N(0, 1)
